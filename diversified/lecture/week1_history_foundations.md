...
1. **수기치료 modality로서의 Diversified HVLA**를 정의하고, osteopathic HVLA(OMT)·물리치료 관절 가동술(MOB)과 구분할 수 있다
...
> **💡 핵심 메시지**
> Diversified HVLA는 **고유한 "에너지 치료"가 아니다**. 기법적 핵심은 **분절 특이적 Motion Palpation 기반 진단**과 **탄성장벽 직전의 고속 저진폭 추력**이라는 **재현 가능한 의료 행위**다. OMT·MD 도수치료의 일부와 **기술적으로 중첩**하며, 상호 참조 가능하다.
...
```mermaid
timeline
    title Diversified의 역사적 궤적
    1895 : D.D. Palmer의 최초 척추 교정 (Davenport, Iowa)
    1897 : Palmer School of Chiropractic 설립
    1906 : B.J. Palmer 학장 취임 — 카이로프랙틱 직업 조직화
    1920s : Straight(철학 중심) vs Mixer(통합 치료) 학파 분기
    1950s-60s : Otto Reinert — National College에서 Diversified 교수법 체계화
    1945-83 : Joe Janse 학장 재임 — Diversified의 현대화·생체역학 기반 재해석
    1970s-80s : Motion Palpation 도입 · 신뢰도 연구 시작
    1990s-2000s : Evidence-Based Chiropractic 운동
    2010-2020s : Cochrane·PMC 다수 systematic reviews 축적 · 모더레이트 근거 확립
```
...
```mermaid
flowchart LR
    A[Neutral<br/>중립] --> B[Active ROM<br/>능동]
    B --> C[Passive ROM<br/>수동]
    C --> D[🟡 Elastic Barrier<br/>탄성 장벽]
    D -- "HVLA Thrust<br/>(100-150 ms)" --> E[🟢 Paraphysiological<br/>Space]
    E --> F[🔴 Anatomical Limit<br/>구조 손상 임계]

    style D fill:#ff9,stroke:#c93,stroke-width:3px
    style E fill:#9f9,stroke:#393,stroke-width:3px
    style F fill:#f99,stroke:#933,stroke-width:3px
```
...
graph TD
    Thrust["HVLA Thrust"]
    Thrust --> M1["🟢 관절 기계수용기<br/>(Type I-IV)<br/>Aβ 섬유 활성"]
    Thrust --> M2["🟢 근방추 활성<br/>γ-motor neuron 재설정"]
    Thrust --> M3["🟢 골지건 기관"]

    M1 --> Spinal["척수 후각<br/>Gate Control"]
    M2 --> Spinal
    M3 --> Spinal

    Spinal --> Pain["통증 전달 억제<br/>(C·Aδ 섬유 입력 감소)"]
    Spinal --> Tone["근긴장 정상화<br/>(α-motor neuron 재조정)"]
    Spinal --> Prop["고유수용감각<br/>재구성"]
...
<figure>
  <img src="./images/wk1_cavitation_mri.jpg" alt="Real-time MRI sequence of MCP joint cavitation — Kawchuk 2015" />
...
> **💡 현장 팁**
> 한국 의학저널·학회 발표에서는 **"고속 저진폭 수기 관절 교정술 (HVLA spinal manipulation)"** 또는 **"분절 기능장애 (segmental dysfunction)"**라는 표현이 오해를 덜 일으키고 정확하다.
...
| 항목 | **Diversified (DC)** | **OMT HVLA (DO)** | **PT MOB** | **MET** |
|------|---------------------|-------------------|------------|---------|
| **수행 주체** | DC | DO | PT | DO/PT |
| **이론 배경** | 생체역학 (modern) | 5 Osteopathic Models | 관절 역학 (Maitland/Mulligan) | 근·근막 tension |
| **진단 도구** | Motion Palpation | Somatic dysfunction TART | Accessory mobility, Maitland grading | 근장력·관절 restrictive barrier |
| **추력 양상** | 고속 저진폭 | 고속 저진폭 | Grade I-IV 진동 (thrust 없음) | 환자 수축 + 수동 교정 |
| **분절 특이성** | 🟢 높음 | 🟢 높음 | 🟢 높음 | 🟢 높음 |
| **근거 수준** | 🟡 Moderate | 🟡 Moderate | 🟡 Moderate | 🟠 Limited |
| **임상 기전 설명** | 🟢 생체역학·신경생리 | 🟢 Somatic dysfunction | 🟢 Arthrokinematics | 🟢 근생리 |

## 6.2 한국 의료 환경에서의 Diversified

```mermaid
flowchart TD
    Patient[통증 환자] --> Consult[정형외과/재활의학과 진료]
    Consult --> Diag{진단}

    Diag -->|기계적 근골격 통증<br/>신경학적 이상 없음| Options{치료 선택}
    Diag -->|Red Flag/<br/>신경학적 결손| Advanced[영상·수술/주사 고려]

    Options --> DC[Diversified HVLA<br/>DC 또는 훈련된 MD]
    Options --> PT[PT 도수치료<br/>MOB·MET·운동재활]
    Options --> Med[약물 + 운동치료]
    DC -.상호 의뢰.-> PT
    DC -.근거 공유.-> Med

    style DC fill:#9f9,stroke:#393
    style PT fill:#9f9,stroke:#393
    style Advanced fill:#fcc,stroke:#933
```

> **💡 의료 전문가 대상 핵심 메시지**
> Diversified HVLA는 **기계적 근골격 통증**의 근거 기반 치료 옵션이며, **한국에서는 정형외과·재활의학과 의사·도수치료 훈련 물리치료사**가 각각의 면허 범위 내에서 활용할 수 있다. 중요한 것은 **기법의 출처(DC/DO/PT)가 아닌 재현 가능한 진단·안전한 실행·근거 기반 적용**이다.
...
```mermaid
flowchart LR
    MD[정형외과/재활의학과<br/>진단·영상·주사·수술]
    PT[물리치료사<br/>도수·운동·modality]
    DC[DC or 훈련된 MD<br/>Diversified HVLA]
    Patient((환자))
    Patient --> MD
    ...
    style DC fill:#9cf,stroke:#339
    style MD fill:#f9c,stroke:#933
```
...
2. 유사 Modality: OMT HVLA(DO), PT MOB(진동 중심), MET(환자 등척성) — **기법적 중첩 있음**
